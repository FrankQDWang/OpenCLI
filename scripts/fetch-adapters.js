...
/**
 * Sparse adapter sync: keeps ~/.seektalent/wtscli/clis/ clean by removing stale overrides.
 *
 ...
 *
 * ~/.seektalent/wtscli/clis/ is a sparse override layer, not a full copy.
 * Only eject-ed or user-modified sites appear here.
 *
 * Only runs on global install (npm install -g) or explicit WTSCLI_FETCH=1.
...
const WTSCLI_DIR = getWtscliConfigDir();
const USER_CLIS_DIR = join(WTSCLI_DIR, 'clis');
const MANIFEST_PATH = join(WTSCLI_DIR, 'adapter-manifest.json');
const PACKAGE_ROOT = resolve(import.meta.dirname, '..');
const BUILTIN_CLIS = join(PACKAGE_ROOT, 'clis');

function log(msg) {
  console.log(`[wtscli] ${msg}`);
}
...
export function fetchAdapters() {
  ...
  // Skip if already installed at the same version (unless forced via WTSCLI_FETCH=1)
  const isForced = process.env.WTSCLI_FETCH === '1';
  ...
  //    Only eject-ed sites live in ~/.seektalent/wtscli/clis/.
  let cleared = 0;
  ...
  // 4. Clean up legacy compat shim files from the WTSCLI-owned state directory.
  // These were created by an older approach that placed re-export shims directly
  // in the WTSCLI state root (e.g., registry.js, errors.js, browser/). The current approach
  ...
  let tmpCleaned = 0;
  try {
    for (const entry of readdirSync(WTSCLI_DIR)) {
      if (entry.startsWith('.plugins.lock.json.tmp-')) {
        try {
          unlinkSync(join(WTSCLI_DIR, entry));
          tmpCleaned++;
        } catch { /* ignore */ }
      }
    }
...
function main() {
  ...
  const isExplicit = process.env.WTSCLI_FETCH === '1';
  const isFirstRun = process.env._WTSCLI_FIRST_RUN === '1';
