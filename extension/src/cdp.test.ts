...
describe('cdp network capture correctness', () => {
  ...
  function createNetworkMock() {
    const onEventListeners: Array<(source: { tabId: number }, method: string, params: unknown) => unknown> = [];
    const debuggerApi = {
      attach: vi.fn(async () => {}),
      detach: vi.fn(async () => {}),
...
describe('cdp evaluateInFrame stale context fallback', () => {
  ...
  it('falls back to the frame target when the cached context id went stale', async () => {
    const debuggerEventListeners: Array<(source: { tabId: number }, method: string, params: unknown) => unknown> = [];
    const debuggerApi = {
      attach: vi.fn(async () => {}),
      detach: vi.fn(async () => {}),
