...
export interface IBrowserFactory {
  connect(opts?: { timeout?: number; session?: string; cdpEndpoint?: string; contextId?: string; preferredContextId?: string; idleTimeout?: number; windowMode?: BrowserWindowMode; surface?: BrowserSurface; siteSession?: 'ephemeral' | 'persistent'; controlKey?: string; fenceToken?: number }): Promise<IPage>;
  close(): Promise<void>;
}
...
export async function browserSession<T>(
  ...
  opts: { session?: string; cdpEndpoint?: string; contextId?: string; preferredContextId?: string; idleTimeout?: number; windowMode?: BrowserWindowMode; surface?: BrowserSurface; siteSession?: 'ephemeral' | 'persistent'; controlKey?: string; fenceToken?: number } = {},
): Promise<T> {
