...
export function readOpenCliSkill(target: string, relpath = '', packageRoot?: string): OpenCliSkillReadResult {
  ...
  if (!name.startsWith('opencli-')) {
    throw new ArgumentError(`Unknown WTSCLI skill: ${name}`, 'Run "wtscli skills list" to see available WTSCLI skills.');
  }
  ...
  if (!fs.existsSync(absolutePath) || !fs.statSync(absolutePath).isFile()) {
    throw new ArgumentError(`Skill file not found: ${name}/${relativePath}`, 'Run "wtscli skills list <skill>" is not supported yet; read SKILL.md or a known references/... file.');
  }
